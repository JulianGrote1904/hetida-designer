...
def is_upward_crossing(previous_value: float, current_value: float, threshold: float, inclusiveness: str) -> bool:
    if inclusiveness == "strict":
        return previous_value < threshold and current_value > threshold
    return previous_value < threshold and current_value >= threshold


def is_downward_crossing(previous_value: float, current_value: float, threshold: float, inclusiveness: str) -> bool:
    if inclusiveness == "strict":
        return previous_value > threshold and current_value < threshold
    return previous_value > threshold and current_value <= threshold
...
def detect_crossings(
    ...
    for idx in range(1, len(series)):
        ...
        upward = is_upward_crossing(float(previous_value), float(current_value), threshold, inclusiveness)
        downward = is_downward_crossing(float(previous_value), float(current_value), threshold, inclusiveness)
...
COMPONENT_INFO = {
    ...
    "version_tag": "1.0.0",
    "id": "c0bb0622-c0e1-4cb7-ab4b-295c41a00ab4",
    "revision_group_id": "c0bb0622-c0e1-4cb7-ab4b-295c41a00ab4",
    "state": "DRAFT",
}
