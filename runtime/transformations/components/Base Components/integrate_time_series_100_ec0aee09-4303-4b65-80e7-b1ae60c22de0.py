...
def calculate_integrated_series_and_period_sums(
    ...
    intervals = build_integration_intervals(
        original_index, series, gap_handling, max_gap
    )

    internal_boundary_timestamps: set[pd.Timestamp] = set()
    for interval_start, interval_end, _, _ in intervals:
        interval_parts = split_interval_at_reset_boundaries(
            interval_start, interval_end, reset
        )
        for _, part_end in interval_parts[:-1]:
            internal_boundary_timestamps.add(part_end)

    integrated_index = original_index.union(
        pd.DatetimeIndex(sorted(internal_boundary_timestamps))
    )
    incremental_area_at_timestamp = {timestamp: 0.0 for timestamp in integrated_index}
    ...
    for timestamp in integrated_index:
        ...
        if reset is not None:
            ...
            if (
                ...
                or timestamp_period_start != current_period_start
            ):
                running_total = 0.0
    ...
    period_sums = pd.Series(
        period_sums_by_period, dtype=float, name="period_sums"
    ).sort_index()
...
COMPONENT_INFO = {
    ...
    "version_tag": "1.0.0",
    "id": "e78e586d-003c-42dd-8bbd-71ec21647e7e",
    "revision_group_id": "ec0aee09-4303-4b65-80e7-b1ae60c22de0",
    "state": "DRAFT",
}
