...
def merge_aligned_values(aligned: pd.DataFrame, merge_operation: str) -> pd.Series:
    ...
    if merge_operation == "left":
        return value_1.astype(float)
    if merge_operation == "right":
        return value_2.astype(float)
    if merge_operation == "sum":
        return (value_1 + value_2).astype(float)
    if merge_operation == "mean":
        return ((value_1 + value_2) / 2.0).astype(float)
    if merge_operation == "min":
        return aligned[["value_1", "value_2"]].min(axis=1, skipna=False).astype(float)
    if merge_operation == "max":
        return aligned[["value_1", "value_2"]].max(axis=1, skipna=False).astype(float)
    return (value_1 - value_2).astype(float)
...
COMPONENT_INFO = {
    ...
    "version_tag": "1.0.0",
    "id": "1c57159c-4880-4c79-bc73-d9864ba0f946",
    "revision_group_id": "1c57159c-4880-4c79-bc73-d9864ba0f946",
    "state": "DRAFT",
}
